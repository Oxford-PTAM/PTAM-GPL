...
VideoSource::VideoSource(std::string inputVideoSourceString)
{
  cout << "  VideoSource_Linux: Opening video source..." << endl;
  if(inputVideoSourceString == "")
  {
    mptr = new VideoCapture(0);
  }
  else
  {
    mptr = new VideoCapture(inputVideoSourceString);
  }


VideoCapture* cap = (VideoCapture*)mptr;
	if(!cap->isOpened()){
	cerr << "Unable to get the video source" << endl;
	exit(-1);
}
...
mirSize = ImageRef(frame.cols, frame.rows);
};
